...
# Page Config
st.set_page_config(
    page_title="Zepto Analytics (Local)",
    page_icon="🟣",
    layout="wide"
)
...
def load_data():
    ...
    if db.client:
        try:
            response = db.client.table("products").select("*").eq("platform", "zepto").execute()
    ...
    if files:
        ...
        try:
            ...
            # Normalize CSV columns to match DB columns for consistency if needed
...
if df.empty:
    st.warning("No Zepto data found in Supabase or local CSVs.")
    st.info("Run `upload_zepto_data.py` to sync CSV data to Supabase.")
    st.info("Or run `run_zepto_assortment.py` to generate local data.")
    st.stop()
...
with tab1:
    ...
    with col_chart1:
        if 'price' in df.columns:
            ...
            fig_price = px.histogram(df, x="price", nbins=20, title="Price Distribution", color_discrete_sequence=['#8E44AD'])
