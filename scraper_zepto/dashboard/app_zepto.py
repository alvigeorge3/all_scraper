...
if scrape_mode == "Single Pincode (Assortment)":
    ...
    if st.sidebar.button("Run Scraper"):
        if not pincode_input:
            ...
            try:
                subprocess.Popen(
                    [sys.executable, "run_zepto.py", "--pincode", pincode_input],
    ...
    if uploaded_file:
        try:
            # Save to root dir as pin_codes.xlsx
            save_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "pin_codes.xlsx")
    ...
    if st.sidebar.button("Run Bulk Assortment"):
        try:
            subprocess.Popen(
                [sys.executable, "run_zepto_assortment_parallel.py"],
                cwd=os.path.abspath(os.path.join(os.path.dirname(__file__), '..')),
                creationflags=subprocess.CREATE_NEW_CONSOLE
            )
    ...
    if uploaded_file:
        try:
            # Save to root dir
            save_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "pin_codes_100.xlsx")
            with open(save_path, "wb") as f:
                f.write(uploaded_file.getbuffer())
            st.sidebar.success("✅ File saved!")
        except Exception as e:
    ...
    if st.sidebar.button("Run Bulk Availability"):
        try:
            subprocess.Popen(
                [sys.executable, "run_zepto_availability_parallel.py"],
                cwd=os.path.abspath(os.path.join(os.path.dirname(__file__), '..')),
                creationflags=subprocess.CREATE_NEW_CONSOLE
            )
