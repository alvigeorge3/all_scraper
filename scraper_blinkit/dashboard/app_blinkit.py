...
st.title("🛍️ Blinkit Assortment & Availability Dashboard")
...
if scrape_mode == "Assortment (All Categories)":
    ...
    if uploaded_file is not None:
        try:
            with open("pin_codes.xlsx", "wb") as f:
    ...
    if st.sidebar.button("Run Assortment Scraper"):
        if not os.path.exists("pin_codes.xlsx"):
             st.sidebar.error("pin_codes.xlsx not found!")
        else:
            subprocess.Popen([sys.executable, "run_blinkit_assortment_parallel.py"], 
    ...
    uploaded_links = st.sidebar.file_uploader("Upload Links & Pincodes Excel", type=['xlsx'], key="availability_uploader")
    if uploaded_links is not None:
        try:
            # The parallel availability runner expects this filename
            with open("pin_codes_100.xlsx", "wb") as f:
                f.write(uploaded_links.getbuffer())
            st.sidebar.success("✅ Saved as pin_codes_100.xlsx")
        except Exception as e:
            st.sidebar.error(f"Error: {e}")

    if st.sidebar.button("Run Availability Scraper"):
        if not os.path.exists("pin_codes_100.xlsx"):
             st.sidebar.error("Input file not found!")
        else:
            subprocess.Popen([sys.executable, "run_blinkit_parallel.py"], 
                             cwd=os.getcwd(), creationflags=subprocess.CREATE_NEW_CONSOLE)
            st.sidebar.success("Availability Scraper started!")
