...
class Database:
    ...
    def save_products(self, products: List[Dict[str, Any]], table_name: str = "zepto_products"):
        """
        Inserts a list of products into the specified table.
        Uses insert to maintain historical records.
        """
        if not self.client:
        ...
        try:
            # Batch Insert (Historical Mode)
            # We use insert instead of upsert to keep history
    ...
    def fetch_products(self, table_name: str = "zepto_products", limit: int = 1000):
        """
        Fetches products from the specified table.
        """
        if not self.client:
            return []
